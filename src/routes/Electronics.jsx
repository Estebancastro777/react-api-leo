    ...
    return (
        <section className="cursive grid grid-cols-2 md:ml-[19%] md:flex md:flex-wrap md:gap-4">
            {filterProducts?.map((products) => (
                <div className="flex flex-col justify-between h-auto w-auto md:h-82 md:w-52 mx-auto p-5 md:rounded-xl gap-10 md:border-t-2 bg-slate-100 hover:border-2 cursor-pointer" key={products.id}>
                    ...
                    <div className="flex gap-2">
                        <button onClick={()=> {addToCart(products);handleAlert(products.id)}} className="flex items-center gap-1 text-sm font-semibold text-white bg-slate-800 hover:bg-slate-700 p-1 px-2 rounded-lg">Agregar al carrito</button>
